    ...
    <v-table fixed-header class="border rounded-sm" height="400px">
      <thead>
        <tr style="background-color: red">
          <th class="text-center">EquipmentID</th>
      ...
      <tbody>
        <tr v-for="maintenance in maintenance_arr" :key="maintenance['_id']">
          <td class="text-center">
            {{ maintenance.equipment["equipment_name"] }}
          </td>
          <td class="text-center">
            {{ maintenance.equipment["equipment_location"] }}
          </td>
...
export default {
  ...
  async mounted() {
    ...
    let maintenance_result = await axios.get(
      `${maintenanceURL}/technician/${user_id}`,
      {
        headers: {
    ...
    for (let maintenance of temp_result_arr) {
      ...
      if (
        maintenance.status != "COMPLETE - SUCCESSFUL" &&
        maintenance.status != "COMPLETE - UNSUCCESSFUL"
      ) {
        temp_maintenance_arr.push(maintenance);
      }
    }

    temp_maintenance_arr.sort((a, b) =>
